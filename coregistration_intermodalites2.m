...
if (Subj_dir(i,1).isdir==1 && exist(t1_path, 'file') ~= 0)
    ...
    system([FSLcommand 'bet ' t1_path ' ' fullfile(Anat_path, Subj_dir(i,1).name, 'Anat', 'ssrAnat.nii') ' -f 0.5 -m']);
%     clear matlabbatch
%     spm_jobman('initcfg');
...
%     clear matlabbatch
     t1_path=fullfile(Anat_path, Subj_dir(i,1).name, 'Anat', 'ssrAnat.nii');
%     disp('---t1 recoupee');
...
%         system([C3Dcommand fullfile(cbf_path, 'cbf_map.nii') ' -resample-mm 1.0x1.0x1.0mm -o ' fullfile(cbf_path, 'c3d_CBF.nii')]);
        system([FSLcommand 'bet ' fullfile(cbf_path, 'c3d_CBF.nii') ' ' fullfile(cbf_path, 'ssc3d_CBF.nii') ' -f 0.5 -m']);
        clear matlabbatch
        spm_jobman('initcfg');
        matlabbatch{1}.spm.spatial.coreg.estwrite.ref = {t1_path};
